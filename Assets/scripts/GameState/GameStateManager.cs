...
{
    // 게임 상태를 vygusgksms rkseksgks dufrjgud
    // Playing : 정상 진행
    // Paused : 일시 정지. timeScale = 0.0f
    // SlowMo : 슬로모션. timeScale 이 0 과 1 사이의 값


    public enum GameState // enum : 열거형. 데이터의 전형을 분류. 여기선 게임 상태 표현을 위해 사용됨.
    { 
    Playing = 0,
    Paused = 1,
    SlowMo = 2
    ...
    //전역 접근이 필요할 수 있으므로 싱글톤처럼 접근할 수 있는 참조 제공. 외부에서도 Instance를 불러와서 클래스의 멤버함수 호출할 수 있게 해줌.
    ...
    //현재 상태를 보관하는 필드
    [SerializeField] private GameState currentState = GameState.Playing; // 현재 게임 플레이 중인 상태로 초기화
    ...
    private void Awake()
    {
        ...
        if (Instance == null)
        {
            Instance = this; //class자신을 Instance에 지정. instance는 반드시 하나만 존재해야함(싱글턴 패턴 : 디자인 패턴의 한 종류)
        }
        else
        {
            if (Instance != this) //Instance는 둘 이상 존재할 수 없기 때문에 삭제시킴.
            { 
                Destroy(gameObject);
                return;
            }
    ...
    //외부에서 Pause 토글을 요청할 때 호출. 일시정지상태일 땐 해제 함수를, 해제 상태일때는 일시정지 함수를 호출
