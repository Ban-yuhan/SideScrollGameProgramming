...
public class SpriteAnimator : MonoBehaviour
{
    ...
    [Header("전환 옵션")]
    [Tooltip("같은 클립을 다시 Play할 떄 프레임 인덱스를 0으로 리셋할지 여부")]
    [SerializeField] private bool resetFrameOnSamePlay = false; //현재 이동 애니메이션을 재생 중일 때, 이동 애니메이션을 또 재생해야한다면 첫 프레임부터 재생할지, 이어서 재생할지 지정
    ...
    // 현재 프레임 인덱스(0 기반)
    private int currentFrameIndex = 0; //현재 재생중인 애니메이션의 몇 번째 스프라이트(프레임)을 출력하고 있는지 저장하기 위한 변수

    //다음 프레임으로 넘어갈 때 까지 남은 시간(초)
    ...
    private void Awake()
    {
        ...
        //시작 시 아무 클립도 재생하지 않으며, 외부에서 Play를 호출하면 그때부터 재생.
    ...
    { 
        ...
        if(currentFrameIndex < 0) //프레임 순서가 정상적이지 않으면 정상 세팅으로 강제
        {
            currentFrameIndex = 0;
        }
