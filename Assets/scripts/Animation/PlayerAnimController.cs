...
///       SpriteAnimator 에 Idle/Move/Jump 중 하나를 재생하도록 지시
///상태 판정 : 
...
{
    //Tooltip : inspector에서 속성에 마우스를 가져다댔을때 뜨는 도움말
    ...
    [Tooltip("바닥 감지에 사용할 레이어 마스크(타일맴/지형)")]
    ...
    [Tooltip("수직 속도가 이 값 이상이면 Jump상태로 간주(양소=상향).")]
