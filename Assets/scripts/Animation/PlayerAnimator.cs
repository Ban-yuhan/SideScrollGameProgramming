...
///  - 이동 방향에 따라 SpriteRenderer.flipX를 갱신.
/// 사용 파라미터(Animator) : 
...
public class PlayerAnimator : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //수평 속도의 절댓값.
        float speedXAbs = Mathf.Abs(speedX); // abs : absolute → 절대값 반환
    ...
    private void ApplyAnimatorParams(float speedAbs, float verticalSpeed, bool grounded)
    {
        ...
        animator.SetFloat(paramSpeed, speedAbs); //speed의 값에 따라 Idle 또는 Move 애니메이션 재싱
        //animator.SetFloat(paramVerticalSpeed, verticalSpeed);
        animator.SetBool(paramisGrounded, grounded); //Set 자료형(파라미터 이름, 변수이름) 애니메이터의 파라미터에 변수의 값을 넣는다는 의미인듯?
    }


    ///<summary>
    ///수평 속도를 기준으로 스프라이트 좌우 반전을 갱신.
    ///flipDeadZone 미만의 미세 속도에서는 반전을 갱신하지 않아 깜빡임 방지
    /// </summary>
    ...
    ///Trigger를 이용해 Jump를 사용할 때 아래와 같이 작성해 호출.
    /// </summary>
