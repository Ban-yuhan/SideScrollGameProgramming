...
//설치 : 아무 오브젝트에도 부착하지 않는 정적(static) 클래스.
//역할 : PlayerPrefs를 사용하여 레벨 별 최단 시간을 저장/불러오기

public static class LevelSaveSystem //static class
{
    //저장 키를 만들 때 접두사를 붙여 충돌 방지
    ...
    public static void SaveBestTime(string levelId, float seconds)
    {
        ...
        float old = PlayerPrefs.GetFloat(key, -.10f); // -1 : 아직 기록 없음
    ...
    public static float GetBestTime(string levelId)
    {
        ...
        string key = MakeBestTimeKey((levelId));
        return PlayerPrefs.GetFloat(key, -1.0f); //저장된 데이터가 없다면 -1.0을 데이터로 사용
