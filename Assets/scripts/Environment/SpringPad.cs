...
//스크립트를 동작시키기 위해서는 Collider2D가 필용
...
public class SpringPad : MonoBehaviour
{
    ...
    [SerializeField] private float targetUpwardSpeed = 12f;
    ...
    [SerializeField] private float maxupwardSpeed = 16f;
    ...
    private float lastBounceTimeUnscaled = -9999f;
    ...
    [SerializeField] private JumpPower jumpPower = JumpPower.Middle;
    ...
    ///<summary>
    ///점프대에 닿은 대상으 ㅣRigidbody2D에 튕김 효과를 적용한다.
    ///</summary>
    ...
    private void TryBounce(Rigidbody2D body)
    {
        ...
        //점프대에 동일 대상이 또 충돌했을 때, 쿨다운이 돌지 않았다면 점프대 작동 X
    ...
    private void ApplyBounce(Rigidbody2D body)
    {
        ...
        //대상의 현재 속도 중 y값을 가져와서 저장
        float vy = v.y;

        //낙하 중이면 추가 보정으로 더 강하게 끌어올린다.
        ...
        // 목표치보다 낮으면 끌어올리고, 높으면 현상태를 유지한다
        if (vy < desired)
        {
            if (jumpPower == JumpPower.Weak)
            {
                vy = desired * 0.5f;
            }

            if (jumpPower == JumpPower.Middle)
            {
                vy = desired;
            }

            if (jumpPower == JumpPower.Strong)
            {
                vy = desired * 1.5f;
            }
