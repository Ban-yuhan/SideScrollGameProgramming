...
public class CameraShake2D : MonoBehaviour
{
    ...
    private void Awake()
    {
        originalLocalPos = transform.localPosition; //카메라의 현재 포지션 저장
        
        //랜덤하게 카메라를 흔들기위해 난수를 발생(시드값 생성)
    ...
    private void LateUpdate()
    {
        if (timeLeft > 0.0f)
        {
            ...
            float t = timeLeft;
            float decay = Mathf.Exp(-damping * (1.0f - (t / Mathf.Max(t, 0.0001f)))); //exp : ()안의 거듭제곱을 구해주는 함수

            float nx = Mathf.PerlinNoise(seedX, Time.unscaledDeltaTime * 50.0f); // PerlinNose : 난수를 생성해주는 함수
            float ny = Mathf.PerlinNoise(seedY, Time.unscaledDeltaTime * 50.0f);
